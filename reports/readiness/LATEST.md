...
`CPI_RELEASE_INFORMATION_RESOLUTION_SPY_F0` (context-only; not an order): assess whether an official 08:30 ET CPI/inflation-release population is a genuinely distinct inflation-surprise/policy-repricing mechanism versus the now-closed FOMC family. Before outcomes freeze official source identity + coverage start, separate candidate-event scope from control-exclusion scope (forbid pre-coverage controls), ambiguous-time stand-aside, next-completed-session entry, predeclared multi-session SPY return geometry with no post-hoc sign flip, prior-only same-regime no-reuse controls, chronological train/sealed unread holdout, dependence-aware uncertainty/tail/density/control-support gates, and zero option pricing at F0. Do not open the sealed FOMC holdout or retune closed FOMC thresholds/horizon/controls/direction/option wrapper. Keep distinct from closed issuer post-earnings drift and other quarantines. Any future option expression needs a separate charter with explicit `capital_fit_usd` / one-lot `max_loss_usd` / `max_lots=1`. No registry, paper force, shadow, arm, broker, or live action. Supersede if orientation finds a higher-information non-quarantined route.

Note (0112 challenger partial): exact FOMC family is `FAMILY_CLOSED` after source-coverage repair + train retest; living surfaces beyond this NEXT line remain the last integrated 0029 state until Sol finalizer refreshes epoch prose and integrates `2026-07-16T0112`.

Last integrated deterministic wrapper postflight (0029): `ok=true`; integrated/pushed/postflight-complete as `76c1a86`; `main == origin/main` at that completion; 0112 run branch not integrated yet.
