# Income strategy coverage — 2026-07-11T1800

Source: `scripts/trader_income_coverage.py` · doctrine: `docs/INCOME_STRATEGY_COVERAGE.md`

- Catalog structures: **17**
- Hypotheses: **201** ({'paper': 1, 'testing': 14, 'candidate': 186})
- Evolve sim artifacts: **54** under `.cache/platform/evolve_backtests/`
...
|---|---|---|---:|---|
| `bear_put_debit_spread` | debit_vertical_sim | bearish_not_bullish | 4 | {'candidate': 4} |
| `bull_call_debit_spread` | debit_vertical_sim | bullish_not_bearish | 7 | {'candidate': 7} |
| `butterfly_spread` | butterfly_sim | neutral_to_bullish_pin | 12 | {'candidate': 12} |
| `calendar_spread` | calendar_sim | time_decay_neutral_to_bullish | 15 | {'candidate': 15} |
| `call_credit_spread` | pcs_sim | prefer_call | 5 | {'candidate': 5} |
| `cash_secured_put` | single_leg | prefer_put | 15 | {'candidate': 15} |
| `diagonal_spread` | diagonal_sim | bullish_time_decay | 7 | {'candidate': 7} |
| `iron_butterfly` | iron_butterfly_sim | neutral_pin_credit | 4 | {'candidate': 4} |
...
- cost realism — normalized observed bid/ask adapter + current yfinance snapshot capture built; no verified no-paid historical chain backfill, so forward archival/join coverage is still missing
