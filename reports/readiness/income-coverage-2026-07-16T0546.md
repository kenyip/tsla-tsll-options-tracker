...
| `put_credit_spread` | pcs_sim | prefer_put | 26 | {'testing': 3, 'candidate': 22, 'rejected': 1} |
...
- cost realism — exact PCS/CCS/IC leg/time join + reject gate, Friday abstraction, archive-backed date-aware expiry/strike-grid provider, and all-expiration atomic append capture built; the current TSLL archive covers 2/3 dates; the density gate still fails closed; the three-date floor is plumbing only, so it cannot establish historical edge or L1; complete observed entry/exit joins and materially deeper history remain required
