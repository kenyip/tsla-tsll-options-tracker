...
- cost realism — exact PCS/CCS/IC leg/time join + reject gate, Friday abstraction, and normalized archive-backed date-aware expiry/strike-grid provider built with coverage counters; required mode fails closed, but one current snapshot covers only one market date and yields zero historical trade-leg matches, so forward archive density and observed-cost calibration remain missing
