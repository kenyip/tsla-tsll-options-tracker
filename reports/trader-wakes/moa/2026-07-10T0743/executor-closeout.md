# MoA EXECUTOR closeout — 2026-07-10T0743
...
Re-run stamp vs prior MoA-exec `2026-07-10T0127` / `0038` (same hyp set). Metrics **reconfirmed identical** within rounding; no new vanity evolve. Mid-RTH day context: earlier 0630/0731 RTH ticks stood aside on b195f5fe (bear_dte=0) — stress is off-hours falsify, not paper fill.

## Hyps under test

| hyp_id | symbol | structure | registry status | capital (registry / stress full) |
|---|---|---|---|---|
| hyp_dna_tsll_put_credit_spread_b195f5fe | TSLL | put_credit_spread | testing | fit_3k ml≈$76.29 max_lots=3; full ml=76.32 |
| hyp_dna_amd_iron_condor_b3056133 | AMD | iron_condor | candidate | fit_3k ml≈$173.7 max_lots=3; full ml=175.47 |
| hyp_dna_xom_call_credit_spread_77766a47 | XOM | call_credit_spread | candidate | fit_3k (stress full ml=202.46); paper multi-leg CCS not fully wired |
...
## Evidence paths (cite these — do not invent metrics)

- `.cache/platform/stress_regime_defined_risk.json` — `at=2026-07-10T14:43:49.363909+00:00`, period=5y, sleeve_usd=3000, symbols=[AMD,TSLL,XOM], structures=[call_credit_spread, iron_condor, put_credit_spread]
- `.cache/platform/stress_cost_defined_risk.json` — `generated_at=2026-07-10T14:43:57.918466+00:00`, slips=[0,0.02,0.05,0.10], `preferred_under_cost=hyp_dna_tsll_put_credit_spread_b195f5fe`
...
- **AMD IC:** 0% SHIP +578.28 → 2% REJECT −965.12 (max_loss≈187.86) → 5% REJECT −2567.58 (max_loss≈200.84) → 10% REJECT −2998.67 (max_loss≈215.7). **Not cost-hold.** Four-leg slip taxes wings; dense n amplifies total_pnl loss while per-trade max_loss stays ~$175–216 fit_3k.
...
- Fresh multi-structure B3/B4 artifacts at stamp **0743** (same paths, new `at`/`generated_at` vs 0127)
- Judgment **unchanged** vs 0127/0038: b195f5fe > XOM CCS >> AMD IC cost_fragile
- No hyp yaml churn; challenger may request surgical evidence_links only if merge wants pins
- Does **not** override today's RTH STAND_ASIDE on TSLL PCS — B6 still needs non-bear live-clock sample

## NEXT SEED (for challenger merge)

1. B6 paper on **b195f5fe OPEN_PCS only** (1-lot), next **non-bear** RTH when filters pass (dry-run → paper place; record max_loss/open_risk or honest stand-aside).
2. Park **XOM CCS** as second path; only paper if multi-leg CCS OPEN is wired — not first paper; keep status=candidate until path ready.
...
Paths written (this file + wake/readiness set in same tick).
