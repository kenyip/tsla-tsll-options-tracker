...
| **Direction / regime bias** | Bull / bear / neutral / stand-aside | Shared-window scoreboard plus `regime_router_sim.py`: one no-lookahead PCS/CCS/IC position with standalone controls; the first router, close-shock PCS, and lagged bullish-momentum PCS walk-forward families were rejected |
