...
2. **Diagonal realism** — scaffold, exact-DNA chronological OOS+density, an injectable announcement-time dividend/short-call assignment guard, and a fail-closed archived Nasdaq declaration-date provider exist; the source is unsupported/ambiguous for some symbols and does not replace observed expiry-specific surfaces
...
4. **Debit-vertical realism** — bull-call/bear-put scaffolds exist; bull-call has the same injectable guard and supported-listing archived declaration-date provider, while observed surfaces and short-put assignment remain
