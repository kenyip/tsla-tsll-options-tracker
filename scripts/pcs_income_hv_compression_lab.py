#!/usr/bin/env python3
"""PCS income pilot V3 — lagged HV compression + non-broken trend; PT50% harvest.

Distinct from:
  - V1 mild-pullback (train pass / holdout fail)
  - V2 Monday-trend (train pass PLTR / holdout fail)
  - Prior closed 14-DTE vol-compression family (different DTE/management bar)

This freezes 21-DTE put credit spreads with 50% profit targets when lag hv_20/hv_60
is compressed and trend is non-broken. Research/proxy L0 only.
"""
...
CANDIDATE_ID = "PCS_INCOME_HV_COMPRESSION_21D_PT50_V1"
FAMILY_ID = "PCS_INCOME_THETA_HARVEST_HV_COMPRESSION"
DEFAULT_SYMBOLS = (
    "BAC",
    "F",
    "SOFI",
...
def income_candidate_config() -> dict[str, Any]:
    """Frozen DNA: sell PCS after lag HV compression while trend not broken."""
    return {
        ...
        "entry_hv_ratio_min": 0.0,
        "entry_hv_ratio_max": 0.85,  # hv_20/hv_60 compressed
        "entry_ema_stack_min": 0.0,
        "entry_ret_14d_max": 0.02,  # not a momentum chase
    }
...
def run_lab(*, symbols: list[str], period: str, train_fraction: float) -> dict[str, Any]:
    ...
    for symbol in normalized:
        try:
            ...
            candidate_axes = _run_axes(symbol, train, candidate_config, "hv_comp_train")
    ...
    if advanced and selected_symbols:
        holdout_block = run_holdout(
            symbols=selected_symbols, period=period, train_fraction=train_fraction
        )
    ...
    return {
        ...
        "family_id": FAMILY_ID,
        "economic_mechanism": (
            "When short-horizon realized vol compresses relative to medium-horizon vol "
            "(lag hv_20/hv_60 <= 0.85) and trend is non-broken, put credit spreads can monetize "
            "range/non-collapse with defined risk; 50% profit targets harvest swings before "
            "vol re-expands."
        ),
        "prior_family_note": (
            "V1 pullback and V2 Monday-trend each advanced on train for thin single-name "
            "edges then failed sealed holdout. Prior 14-DTE vol-compression rolling-origin "
            "family is a different DTE/management claim and remains closed on its own terms."
        ),
        "funnel_stage_before": "F0_MECHANISM",
        "funnel_stage_after": (
        ...
        "next_seed_if_closed": (
            "PCS_INCOME_POST_SHOCK_STABILIZE_21D_PT50_V1: keep PT50% management; predeclare "
            "entry after a completed adverse shock day with subsequent stabilization "
            "(lagged), distinct from HV-ratio compression and calendar filters; or pause "
            "adjacent PCS entry mining if three consecutive holdout fails imply diminishing returns."
        ),
        "errors": errors,
        "rows": rows,
    }
