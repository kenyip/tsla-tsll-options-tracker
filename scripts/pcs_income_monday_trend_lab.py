#!/usr/bin/env python3
"""PCS income pilot V2 — Monday entries in non-broken trend; PT50% harvest.

Distinct from mild-pullback V1 (which passed train dual-cost on PLTR/SMCI but
failed sealed holdout). Research/proxy L0 only. Cannot earn L1 or trading authority.
"""
...
CANDIDATE_ID = "PCS_INCOME_MONDAY_TREND_21D_PT50_V1"
FAMILY_ID = "PCS_INCOME_THETA_HARVEST_MONDAY_TREND"
DEFAULT_SYMBOLS = (
    "BAC",
    "F",
    "SOFI",
...
def income_candidate_config() -> dict[str, Any]:
    """Frozen DNA: Monday-only PCS when lag trend non-broken and 5d return not extended."""
    return {
...
def run_holdout(*, symbols: list[str], period: str, train_fraction: float) -> dict[str, Any]:
    """Evaluate frozen candidate DNA on untouched holdout only (no ranking/retune)."""
...
def run_lab(*, symbols: list[str], period: str, train_fraction: float) -> dict[str, Any]:
    ...
    for symbol in normalized:
        try:
            ...
            candidate_axes = _run_axes(symbol, train, candidate_config, "monday_trend_train")
    ...
    return {
        ...
        "family_id": FAMILY_ID,
        "economic_mechanism": (
            "Monday session entries after completed non-broken trend filters may avoid "
            "midweek chase fills; 21-DTE put credit spreads with 50% profit targets harvest "
            "theta/mean-reversion while standing aside pure bear regimes."
        ),
        "prior_family_note": (
            "Mild-pullback V1 advanced on train for PLTR/SMCI but failed sealed holdout; "
            "this family changes only entry timing/filters, not PT/DTE management polish."
        ),
        "funnel_stage_before": "F0_MECHANISM",
        "funnel_stage_after": (
        ...
        "next_seed_if_closed": (
            "PCS_INCOME_HV_COMPRESSION_21D_PT50_V1: keep PT50% management; predeclare "
            "lagged hv_20/hv_60 compression + non-broken trend entry distinct from Monday "
            "calendar filter; or stand aside and wait for denser multi-name dual-cost signal."
        ),
        "errors": errors,
        "rows": rows,
    }
